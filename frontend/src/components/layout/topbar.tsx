...
import { savePosSettings, useCurrency, type CurrencyCode } from "@/lib/currency/provider";
...
export function Topbar({ onOpenMobileSidebar, className }: TopbarProps) {
  ...
  const { settings, currency } = useCurrency();
  ...
  function handlePickItem(item: InventoryListItem) {
    const mappedItem = mapToInventoryRow(item);
    navigate(`/inventory/${item.id}`, { state: { item: mappedItem } });
    setIsSuggestionsOpen(false);
  }
      ...
      <div className="mx-auto flex w-full flex-wrap items-start gap-3 rounded-3xl border border-muted/40 bg-muted/30 px-4 py-3 shadow-sm lg:flex-nowrap lg:items-center">
        ...
        <div className="order-2 ml-auto flex items-center gap-2 lg:order-none lg:ml-0">
          <Select value={currency} onValueChange={(value) => handleCurrencyChange(value as CurrencyCode)}>
            <SelectTrigger
              className="h-10 w-[88px] rounded-2xl border-muted/40 bg-white dark:bg-muted/20 dark:border-muted/30"
