...
import { tokenVersionMatches } from '../shared';
...
export class JwtStrategy extends PassportStrategy(Strategy, 'jwt') {
  ...
  async validate(payload: JwtPayload): Promise<JwtRequestUser> {
    // Har requestda token ichidagi sub bo'yicha aktiv userni tasdiqlaymiz.
    ...
    // Guardlar uchun minimal request user payload qaytaramiz.
    return {
      id: payload.sub,
      sub: payload.sub,
      role: payload.role,
